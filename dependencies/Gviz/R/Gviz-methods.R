...
setMethod("drawGD", signature("DataTrack"), function(GdObject, minBase, maxBase, prepare=FALSE, subset=TRUE, ...) {
	...
	if (("a_confint" %in% type) && (!"a" %in% type)) type <- c(type,"a")
	...
	plot_args[["x"]] <- x; plot_args[["y"]] <- y; plot_args[["subscripts"]] <- seq_along(x);
		...
		if (!is.null(groups)){
			...
			for (j in seq_along(by)) {
				g <- names(by)[j]
					...
					curplot <- xyplot(y~x, data=df, groups=groups,fill=fill[j],
						upper=df$high, lower=df$low,
						panel = function(x,y,...) {
							panel.superpose(x,y,panel.groups=my.panel.bands,type="l",...)
							do.call("panel.xyplot",plot_args)
						},ylim=c(minnie,maxie),
						scales=list(
							x=list(relation="same",draw=FALSE),
							y=list(relation="same",draw=FALSE)
							),
						par.settings=list(axis.line=list(col=0)),
						xlab=NULL,ylab=NULL
					#xlim=c(minnie,maxie), scales=list(x="same",y="same")
				) # the newpage flag needs to be set to FALSE to avoid popping the viewport before we are ready to do so.
				if (is.null(outg)) {
					outg <- curplot
				} else {
					outg <- outg + as.layer(curplot)
#do.call("panel.xyplot",plot_args)
				}
			}
			cat(sprintf("min=%1.2f, max=%1.2f\n",minnie,maxie))
				...
				confint <- 1.96*(apply(vals,2, sd)/sqrt(nrow(vald)))
				df <- data.frame(x=position(GdObject), y=mu, low=mu-confint, high=mu+confint,groups=factor(1));
				print(xyplot(y~x, data=df, groups=groups,
					upper=df$high, lower=df$low,
					panel = function(x,y,...) {
						panel.superpose(x,y,panel.groups=my.panel.bands,type="l",fill=fill,...)
						do.call("panel.xyplot",plot_args)
					},
					newpage=FALSE)) # the newpage flag needs to be set to FALSE to avoid popping the viewport before we are ready to do so.
		}
...
##---------------------------------------------------------------------------------
# Helper methods for fast computation of row-wise SD
###---------------------------------------------------------------------------------
#.getSD <- function# get row-wise standard deviation
#(m ##<< (matrix) rows are values, columns are individual samples.
#) {
